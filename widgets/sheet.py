...
import pyui2
from pyui2.desktop import getTheme
from pyui2.base import Base
from pyui2.panel import Panel
from pyui2.window import Window
from pyui2.widgets.edit import Edit


class SheetEntry(Edit):
    ...
    def activateAt(self, col, row):
        ...
        self.setShow(1)
        self.moveto(self.sheet.posX + x + 1, self.sheet.posY + y)
        self.resize(w-2,h)
...
class Sheet(Base):
    ...
    def __init__(self, changeHandler=None, insertHandler=None):
        ...
        self.registerEvent(pyui2.locals.LMOUSEBUTTONDOWN, self.onLMouseDown)
        self.registerEvent(pyui2.locals.LMOUSEBUTTONUP, self.onLMouseUp)
        self.registerEvent(pyui2.locals.MOUSEMOVE, self.onMouseMove)
    ...
    def resize(self, width, height):
        pyui2.widgets.Base.resize(self, width, height)
        self.vscroll.moveto(self.posX + self.width - 12, self.posY)
        self.vscroll.resize(12, self.height)
        self.hscroll.moveto(self.posX, self.posY + self.height - 12)
        self.hscroll.resize(self.width, 12)
    ...
    def calcSize(self):
        pyui2.widgets.Base.calcSize(self)
...
#        renderer.drawRect(pyui2.colors.white, self.windowRect)
...
#                              self.windowRect[1] + self.windowRect[3]-12, pyui2.colors.grey)
#
...
#                          self.windowRect[0] + posX - 1, self.windowRect[1] + self.windowRect[3]-12, pyui2.colors.grey)
#
...
#                              self.windowRect[1] + posY, pyui2.colors.grey)
#
#            posY = posY + h
#
#        renderer.drawLine(self.windowRect[0],self.windowRect[1] + posY,
#                          self.windowRect[0] + self.windowRect[2]-12, self.windowRect[1] + posY, pyui2.colors.grey)
#
#        # draw cell contents
#        posY = self.getRowHeight(self.vscroll.currentItem)
#        for y in range(1, self.visibleRows):
...
#                if value:
#                    renderer.drawText( "%s"%value, (self.windowRect[0]+posX+2, self.windowRect[1]+posY), pyui2.colors.black)
