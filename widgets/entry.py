...
from pyui2.desktop import getTheme, getDesktop, getPresenter
...
#       return rect
    ...
    def getGrid( self ):
        ...
        font_width, font_height = self.font.getTextSize(' ')
        return int(float(w-16) / float(font_width)), int(float(h-16) / float(font_height))
    ...
    def findMousePos(self, pos):
        ...
        row = max( math.floor( (float(y) / float(font_height)) ) + self.scroll[1], 0 )
    ...
    def _pyui2KeyUp(self, event):
        ...
        if event.key == pyui2.locals.K_SHIFT:
            self.shifting = 0
            return 1
