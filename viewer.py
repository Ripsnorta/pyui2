...
import types
import pyui2
from pyui2.widgets import Tree, TreeNode
from pyui2.locals import *

class Viewer(pyui2.widgets.Frame):

    def __init__(self, x, y, target):
        pyui2.widgets.Frame.__init__(self, x, y, 400, 200, "%s" % target)
        if not target:
        ...
        self.typeTabs = pyui2.widgets.TabbedPanel()
        ...
        self.numPanel.setLayout(pyui2.layouts.GridLayoutManager(1, len(self.numPanel.children)+1) )
        self.stringPanel.setLayout(pyui2.layouts.GridLayoutManager(1, len(self.stringPanel.children)+1) )
        self.instancePanel.setLayout(pyui2.layouts.GridLayoutManager(1, len(self.instancePanel.children)+1) )
        self.collectionsPanel.setLayout(pyui2.layouts.GridLayoutManager(1, len(self.collectionsPanel.children)+1) )        
        self.otherPanel.setLayout(pyui2.layouts.GridLayoutManager(1, len(self.otherPanel.children)+1) )

        self.typeTabs.resize(398,398)
        self.addChild(self.typeTabs, pyui2.locals.CENTER)
        self.resize(400, 400)
        self.pack()
        self.valid = 1
...
class NumberViewer(pyui2.widgets.Panel):
    def __init__(self, target, attributeName, value):
        self.target = target
        self.attributeName = attributeName
        self.value = value
        pyui2.widgets.Panel.__init__(self)
        self.setLayout(pyui2.layouts.GridLayoutManager(3,1))
        self.label = pyui2.widgets.Label(attributeName)
        self.typeLabel = pyui2.widgets.Label(repr(type(value)))
        self.editValue = pyui2.widgets.Edit("%d" % value, 64, self.onEnter)
...
class StringViewer(pyui2.widgets.Panel):
    def __init__(self, target, attributeName, value):
        self.target = target
        self.attributeName = attributeName
        self.value = value
        ...
        self.label = pyui2.widgets.Label(attributeName)
        self.typeLabel = pyui2.widgets.Label(repr(type(value)))                
        self.editValue = pyui2.widgets.Edit(value, 64, self.onEnter)
        self.addChild(self.label)
        self.addChild(self.typeLabel)
        self.addChild(self.editValue)
        self.pack()
...
class InstanceViewer(pyui2.widgets.Button):
    def __init__(self, target, attributeName, value):
        self.target = target
        self.attributeName = attributeName
        self.value = value
        pyui2.widgets.Button.__init__(self, repr(value), self.onButton)

    def onButton(self, button):
        newViewer = Viewer(self.posX + 30, self.posY + 30, self.value)

class NoneViewer(pyui2.widgets.Label):
    def __init__(self, attributeName):
        pyui2.widgets.Label.__init__(self, "%s: None" % attributeName)

class ListViewer(pyui2.widgets.Panel):
    def __init__(self, attributeName, value):
        self.attributeName = attributeName
        self.value = value
        pyui2.widgets.Panel.__init__(self)
        self.setLayout(pyui2.layouts.GridLayoutManager(2, 1))

        self.box = pyui2.widgets.ListBox()
        for v in self.value:
            self.box.addItem(repr(v), v)
        self.button = pyui2.widgets.Button("view %s" % attributeName, self.onView)
...
class DictViewer(pyui2.widgets.Panel):
    def __init__(self, attributeName, value):
        self.attributeName = attributeName
        self.value = value
        pyui2.widgets.Panel.__init__(self)
...
class DefaultViewer(pyui2.widgets.Panel):
    def __init__(self, attributeName, value):
        self.attributeName = attributeName
        self.value = value
        pyui2.widgets.Panel.__init__(self)
        self.setLayout(pyui2.layouts.GridLayoutManager(3,1))        
        self.label = pyui2.widgets.Label("%s:" % attributeName)
        self.typeLabel = pyui2.widgets.Label(repr(type(value)))        
        self.editValue = pyui2.widgets.Label(repr(value))
        self.addChild(self.label)
        self.addChild(self.typeLabel)
        self.addChild(self.editValue)
        self.pack()
...
class TreeViewerWindow(pyui2.widgets.Frame):
    """A tree widget based python object viewer.
    """

    def __init__(self, x, y, w, h, target):
        pyui2.widgets.Frame.__init__(self, x, y, w, h, repr(target))
        self.target = target
...
class ViewerNode(TreeNode):
    def __init__(self, title, target):
        ...
        TreeNode.__init__(self, title, bitmap)
