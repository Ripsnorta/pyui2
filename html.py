...
class pyui2HTMLParser(HTMLParser):
...
on creation and resize, build a set of primitives that can draw themselves. this includes
wrapping text with appropriate fonts. these primitive will be arranged into lines in drawing
order left to right.

primitives in a line do not overlap to the right of the panel.

each line has a height. primitives may be aligned vertically within the line's height.
...
class HTMLPanel(pyui2.base.Panel):
    """Panel that displays html.
    """
    
    def __init__(self):
        self.htmlText = ""
        pyui2.base.Panel.__init__(self)
        self.parser = pyui2HTMLParser(self)
        self.root = None
        ...
        self.bgColor = pyui2.colors.white
        self.textColor = pyui2.colors.black
        self.scrollbar = pyui2.widgets.VScroll()
        self.addChild(self.scrollbar)
        self.loadFormatElements()
        self.loadVisibleElements()
        
    def loadFormatElements(self):
    ...
    def buildPrimitives(self):
        """This constructs the set of primitives for drawing the HTML
        elements in this panel.
        """
        self.areas = [PrimitiveArea([], self.windowRect[2], self.windowRect[3], self.fgColor)]
        self.formatStack = []
        self.attrsStack = []
        ...
        for line in self.areas[0].lines:
            if len(line.primitives):
                print line.primitives[0].text
                if isinstance(line.primitives[0], PrimitiveTable):
                    for l2 in line.primitives[0].lines:
                        for p in l2.primitives:
                            print "    ", p.text
    ...
    def expandToken(self, token):
        """This does the work of constructing lines and primitives from
        ...
        if isinstance(token, HTMLTag):
            ...
            if visibleElement:
                ...
                if token.tag == 'table':
                    ...
                    self.areas[-1].addPrimitive(newPrim)
            ...
            if token.tag == 'table':
                ...
                self.areas[-1].addPrimitive(table)
...
visibleElements = [
    ...
    ('img', 0,     0,    ImagePrimitive, "Image"),
    ('hr',  1,     1,    HRPrimitive,    "Horizontal Line"),
    ('table',1,    1,    PrimitiveTable, "Table"),
