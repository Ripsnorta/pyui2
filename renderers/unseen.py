...
from pyui2.renderer3d import Renderer3DBase
...
def gotEvent(event, wParam, lParam):
    ...
    if event == WM_CHAR:
        getDesktop().postUserEvent(pyui2.locals.CHAR, 0, 0, chr(wParam), mods)
        return
    ...
    if event == WM_KEYDOWN:
        ...
        keystate[wParam] += 1
        getDesktop().postUserEvent(pyui2.locals.KEYDOWN, 0, 0, wParam, mods)
        return

    if event == WM_KEYUP:
        global keydown, keystate
        keystate[wParam] = 0
        getDesktop().postUserEvent(pyui2.locals.KEYUP, 0, 0, wParam, mods)
        return
        
    # special event handlers
    if event == WM_CLOSE:
        getDesktop().postUserEvent(pyui2.locals.QUIT, 0, 0, 0)
        return
...
class Unseen(Renderer3DBase):
    ...
    def populateConstants(self):
        """Populate pyui2.constants with the values from msgs.py which are win32 message types.
        """
        pyui2.locals.LMOUSEBUTTONDOWN    = WM_LBUTTONDOWN
        pyui2.locals.RMOUSEBUTTONDOWN    = WM_RBUTTONDOWN
        pyui2.locals.MMOUSEBUTTONDOWN    = WM_MBUTTONDOWN
        pyui2.locals.LMOUSEBUTTONUP      = WM_LBUTTONUP
        pyui2.locals.RMOUSEBUTTONUP      = WM_RBUTTONUP
        pyui2.locals.MMOUSEBUTTONUP      = WM_MBUTTONUP
        pyui2.locals.MOUSEMOVE           = WM_MOUSEMOVE
        pyui2.locals.MOUSEWHEEL          = WM_MOUSEWHEEL
        pyui2.locals.QUIT                = WM_CLOSE
        pyui2.locals.LMOUSEDBLCLICK       = WM_LBUTTONDBLCLK
        pyui2.locals.RMOUSEDBLCLICK       = WM_RBUTTONDBLCLK
        pyui2.locals.MMOUSEDBLCLICK       = WM_MBUTTONDBLCLK        

        global keydown
        pyui2.locals.K_BACKSPACE = VK_BACK
        pyui2.locals.K_TAB       = VK_TAB
        pyui2.locals.K_RETURN    = VK_RETURN
        pyui2.locals.K_SHIFT     = VK_SHIFT
        pyui2.locals.K_CONTROL   = VK_CONTROL
        pyui2.locals.K_ALT       = VK_MENU
        pyui2.locals.K_ESCAPE    = VK_ESCAPE
        pyui2.locals.K_SPACE     = VK_SPACE
        pyui2.locals.K_PAGEUP    = VK_PRIOR
        pyui2.locals.K_PAGEDOWN  = VK_NEXT
        pyui2.locals.K_END       = VK_END
        pyui2.locals.K_HOME      = VK_HOME
        
        pyui2.locals.K_LEFT      = VK_LEFT
        pyui2.locals.K_RIGHT     = VK_RIGHT
        pyui2.locals.K_UP        = VK_UP
        pyui2.locals.K_DOWN      = VK_DOWN

        pyui2.locals.K_INSERT    = VK_INSERT
        pyui2.locals.K_DELETE    = VK_DELETE

        pyui2.locals.K_F1        = VK_F1
        pyui2.locals.K_F2        = VK_F2
        pyui2.locals.K_F3        = VK_F3
        pyui2.locals.K_F4        = VK_F4
        pyui2.locals.K_F5        = VK_F5
        pyui2.locals.K_F6        = VK_F6
        pyui2.locals.K_F7        = VK_F7
        pyui2.locals.K_F8        = VK_F8
        pyui2.locals.K_F9        = VK_F9
        pyui2.locals.K_F10       = VK_F10
        pyui2.locals.K_F11       = VK_F11
        pyui2.locals.K_F12       = VK_F12
    ...
    def drawRect(self, color, rect):
        ...
        if rect[2] == 0 or rect[3] == 0:
            return
        self.drawList.append( (pyui2.locals.RECT, rect[0], rect[1], rect[2], rect[3], color ) ) 
    ...
    def drawText(self, text, pos, color, font = None):
        ...
        if font == 'fixed':
            ...
            font = self.font1
        self.drawList.append( (pyui2.locals.TEXT, pos[0], pos[1], color, font, text))
    ...
    def drawGradient(self, rect, c1, c2, c3, c4):
        ...
        if rect[2] == 0 or rect[3] == 0:
            return
        self.drawList.append( (pyui2.locals.GRADIENT, rect[0], rect[1], rect[2], rect[3], c1, c2, c3, c4) )
    ...
    def drawImage(self, rect, filename, pieceRect = None):
        ...
        if not pieceRect:
            pieceRect = (0,0,1,1)
        self.drawList.append( (pyui2.locals.IMAGE, rect[0], rect[1], rect[2], rect[3], self.images[filename], 0,
    ...
    def drawImageRotated(self, rect, filename, rotation=0, textureEffect=0 ):
        ...
        if not self.images.has_key(filename):
            self.loadImage(filename)
        self.drawList.append( (pyui2.locals.IMAGE, rect[0], rect[1], rect[2], rect[3], self.images[filename], rotation) )
    ...
    def drawView(self, rect, handle):
        ...
        self.drawList.append( (pyui2.locals.VIEW, rect[0], rect[1], rect[2], rect[3], handle) )
    ...
    def setClipping(self, rect = None):
        ...
        #self.drawList.append( [pyui2.locals.CLIP, (rect[0], rect[1], rect[2], rect[3]) ] )
        pass
